...
def solve_naive_array_flat(capacity, items, weights, values):
    ...
    for item in range(items):
        grid[(item + 1) * (capacity + 1):(item + 2) * (capacity + 1)] = grid[item * (capacity + 1):(item + 1) * (
                    capacity + 1)]
        for k in range(weights[item], capacity + 1):
            grid[(item + 1) * (capacity + 1) + k] = max(grid[item * (capacity + 1) + k],
                                                        grid[item * (capacity + 1) + k - weights[item]] + values[item])
...
def solve_map(capacity, items, weights, values):
    ...
    for item in range(items):
        ...
        grid[item + 1][this_weight:] = list(map(lambda k: max(grid[item][k], grid[item][k - this_weight] + this_value),
...
def solve_list_comp(capacity, items, weights, values):
    ...
    for item in range(items):
        ...
        grid[item + 1][this_weight:] = [max(grid[item][k], grid[item][k - this_weight] + this_value) for k in
                                        range(this_weight, capacity + 1)]
...
def solve_numpy_func_buffer(capacity, items, weights, values):
    ...
    for item in range(items):
        ...
        grid[item + 1, this_weight:] = np.where(temp[:-this_weight] > grid[item, this_weight:], temp[:-this_weight],
